...
max_iters = 9000  # train dataset size
epochs = 3
weight_decay = 0.01
lora_r = 8
lora_alpha = 16
...
# wandb.init(project="neurips_training", config=hparams)
...
def generate_prompt(example: dict) -> str:
    ...
    if example["input"]:
        return (
            f"[INST] {example['instruction']} {example['input']} [/INST]"
        )
    return (
...
def setup(
    data_dir: Path = Path("./data/lima-platypus-final"),
    ...
    if devices > 1:
        ...
        # strategy = "auto"
        strategy = SingleDeviceProdStrategy(device=torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu"))
...
def main(fabric: L.Fabric, data_dir: Path, checkpoint_dir: Path, out_dir: Path, quantize: Optional[str] = None) -> None:
    ...
    if isinstance(fabric.strategy.precision, BitsandbytesPrecision):
        ...
        optimizer = bnb.optim.Lion8bit(trainable_params, lr=learning_rate, weight_decay=weight_decay)
    else:
...
def train(
    ...
    for iter_num in range(max_iters):
        ...
        input_ids, targets = get_batch(fabric, train_data, iter_num, longest_seq_ix)
        ...
        if not is_accumulating and step_count % eval_interval == 0:
            ...
            if val_loss.item() < best_val_loss:
                ...
                checkpoint_path = out_dir / f"best-ckpt.pth"
        ...
        del logits  # after their last use
...
def validate(fabric: L.Fabric, model: GPT, val_data: List[Dict], tokenizer: Tokenizer) -> torch.Tensor:
    ...
    for k in range(eval_iters):
        input_ids, targets = get_batch(fabric, val_data, k)
        logits = model(input_ids)
        losses[k] = chunked_cross_entropy(logits[..., :-1, :], targets[..., 1:], chunk_size=0)
        del input_ids, targets  # after their last use
        gc.collect()
        
    val_loss = losses.mean()

    # produce an example:
    prompt = "[INST] Recommend a movie for me to watch during the weekend and explain the reason. [/INST]"
    fabric.print(prompt)
    encoded = tokenizer.encode(prompt, device=fabric.device)
    with fabric.init_tensor():
        # do not set `max_seq_length=max_returned_token` because memory is not a concern here
        model.set_kv_cache(batch_size=1)
    output = generate(model, encoded, max_returned_tokens=len(encoded) + eval_max_new_tokens, temperature=0.8, eos_id=2)
    model.clear_kv_cache()
    output = tokenizer.decode(output)
    fabric.print(output)
    
    # wandb.log({f"output_{OUTPUT_COUNTER}": output})
    OUTPUT_COUNTER += 1
...
def get_batch(
    fabric: L.Fabric, data: List[Dict], idx: int, longest_seq_ix: Optional[int] = None
) -> Tuple[torch.Tensor, torch.Tensor]:
    start_ix = (idx * micro_batch_size) % len(data)
    end_ix = start_ix + micro_batch_size
    ix = [i % len(data) for i in range(start_ix, end_ix)]
    
    if longest_seq_ix is not None and idx == 0:
        # force the longest sample at the beginning so potential OOMs happen right away
        ix[0] = longest_seq_ix
    elif longest_seq_ix is not None and longest_seq_ix in ix:
        ix.remove(longest_seq_ix)
        ix.insert(0, 0)
